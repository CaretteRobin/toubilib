...
try {
    ...
    echo $user->verifyPassword('password123') ? "✓ Mot de passe correct" : "✗ Erreur mot de passe";
    echo "\n";
    echo $user->verifyPassword('wrongpassword') ? "✗ Erreur sécurité" : "✓ Mot de passe incorrect rejeté";
    echo "\n";
    ...
    $authService = new ServiceAuth(
        new PDOUserRepository($mockPdo),
        $jwtSecret,
        3600
    );

    echo "\nTest 5: Génération JWT\n";
    $token = $authService->generateJwtToken($dto);
    echo "✓ Token JWT généré (longueur: " . strlen($token) . " caractères)\n";
